...
2
# 3-2.LSTM
![1](images_Deeplearning_Latter/3_2_1.png)  
RNNの一種.RNNでは時系列を過去に遡るほど勾配が消失する（勾配消失問題）.LSTMは、ニューラルネットワークの構造を変えて解決したもの.
## CEC
RNNの中間層の肝を抽出した機能.過去の情報を全て記憶する.重みが一律である為、学習機能は無い.入力ゲートと出力ゲートと忘却ゲートで調節する.
## 入力ゲート
CECに対して、どれを覚えさせるかを学習する
## 出力ゲート
CECに対して、どれを使用するかを学習する
## 忘却ゲート
CECは、過去の情報を全て記憶する.過去の情報が要らなくなった時に、忘却させる機能.
## 覗き穴結合
CEC自身の値に重み行列を介して伝播可能にした構造.判断材料が増えたということにはなるが、
あまり大きな効果は見られなかった.

![1](images_Deeplearning_Latter/3_2_2.png)  

3
# 3-3.GRU
![1](images_Deeplearning_Latter/3_3_1.png)  
LSTMの改良版.LSTMはパラメータが多く、計算負荷が高くなる問題があった.そこで、GRUでは構造を変えることでパラメータを削減し、計算量を少なくかつタスクによっては精度も同様またはそれ以上を望めるようになった.
## リセットゲート
隠れ層の状態をどのような状態で保持しておくかを制御する.

## 更新ゲート
どのように今覚えてることを使って出力を得るかを制御する.

![1](images_Deeplearning_Latter/3_3_2.png)  

4
# 3-4.双方向RNN
![1](images_Deeplearning_Latter/3_4_1.png)  
通常のRNNは過去の情報を保持することで時系列データの学習を実現していたが、双方向RNNは過去の情報に加えて未来の情報も加味させるモデル.

![1](images_Deeplearning_Latter/3_4_2.png)  

4

# 3-5.Seq2Seq(ここからRNNの自然言語処理)
...
![1](images_Deeplearning_Latter/3_6_1.png) 
...
![1](images_Deeplearning_Latter/3_7_1.png) 
...
Attention Mechanism：時系列データに対して関連性に重みをつける手法
